...
def student_id(result):
    ...
    if result in list:
        #유닛테스팅을 위한 코드 수정. 기존 : print(list[result])
        return True
    else:
        return False
...
class TestCapErrorFinder(unittest.TestCase):
    def test_error_finder_1(self):
        input = "5/9+4"
        self.assertEqual("입력은 정수 또는 지정된 연산자(+,-,*)만 입력가능합니다.", error_finder(input))
    def test_error_finder_2(self):
        input = "+9*11"
        self.assertEqual("연산자가 처음으로 입력되었습니다.", error_finder(input))
    def test_error_finder_3(self):
        input = "9*+11"
        self.assertEqual("연산자 중복오류", error_finder(input))
